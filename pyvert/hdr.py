...
class HdrPatcher():
    ...
    def __init__(self, infile, outfile, options):
        self.INFILE = infile
        self.OUTFILE = outfile
        
        #MaxCLL parameter to be written into SEI
        self.maxcll = options['MAXCLL']
        
        #Primaries to be put into VUI section. Default = 9 (bt2020)
        if options['COLORPRIM'] in self.prim_list:
            self.primaries = self.prim_list.index(options['COLORPRIM'])
        
        #Transfer function describing curve. Default = 16 (SMPTE ST2084 curve)
        if options['TRANSFER'] in self.trc_list:
            self.trc = self.trc_list.index(options['TRANSFER'])     
        
        #Color matrix. Default = 9 (bt2020nc)
        if options['COLORMATRIX'] in self.mtx_list:
            self.matrix = self.mtx_list.index(options['COLORMATRIX'])
        
        #Chroma bit location = 2 (as required by UHD BD specs)
        if options['CHROMALOC'] in range(6):
            self.chroma_bit = options['CHROMALOC']

        #Video format (COMPONENT,PAL, NTSC, e.t.c.) Default = 5 (Unspecified)
        if options['VIDEOFORMAT'] in self.vid_fmt_list:
            self.video_fmt = self.vid_fmt_list.index(options['VIDEOFORMAT'])
            
        #Video range Default = 0 (TV)
        if options['FULL_RANGE'] in self.range_list:
            self.full_range = self.range_list.index(options['FULL_RANGE'])
            
        self.md_arg_str = options['MASTERDP']
    ...
    def process(self):
        ...
        if self.md_arg_str:
            md = re.findall('\d+',self.md_arg_str)
            if len(md) != 10 :
                logger.error('Specified wrong "-masterdisplay" parameter!')
                exit(1)

        if self.maxcll:
            mcll = re.findall('\d+',self.maxcll)
        sei_ok = 0 
        
        F = open (self.INFILE,'r+b')
        o = open (self.OUTFILE,'wb')

        print('HDR: Prepending SEI data')
        s = BitStream(F.read(self.chunk))
        ...
        ### MAXCLL & MAXFALL ###
        if self.maxcll or self.md_arg_str:
            ...
            print('HDR: Starting new SEI NALu...')
            ...
            if self.maxcll:
                ...
                print('HDR: SEI message with MaxCLL=',sei_max_content_light_level,' and MaxFall=',sei_max_pic_average_light_level,' created in SEI NAL')
            ...
            if self.md_arg_str:
                ...
                print('HDR: SEI message Mastering Display Data {} created in SEI NAL'.format(self.md_arg_str))     
        ...
        print('HDR: Looking for SPS.........', sps_pos)
        print('HDR: SPS_Nals_addresses', sps_pos)
        print('HDR: SPS NAL Size', sps_size)
        print('HDR: Starting reading SPS NAL contents')
        ...
        if vui_parameters_present_flag :
           vp = vui_parameters(t,self.chroma_bit,self.video_fmt,self.full_range,self.primaries,self.trc,self.matrix)
           vp.show()
        ...
        print('HDR: Reading of SPS NAL finished. Read ',len(t),' of SPS NALu data.\n')
        
        # New BS write Block
        print('HDR: Making modified SPS NALu...')
        new_bs = BitStream()
        ...
        if vui_parameters_present_flag :
           ...
           if vp.video_signal_type_present_flag :
               ...
               print("--------------")
               print(vp.colour_description_present_flag)
               print("--------------")
        ...
        print('HDR: Made modified SPS NALu - OK')
        if sei_ok :
            s.prepend (new_sei_string)
            print('HDR: New SEI prepended')
        print('HDR: Writing new stream...')
        s.tofile(o)
        progr = self.chunk
        while True:
            s = F.read(self.chunk)
            ...
            if progr < os.path.getsize(self.INFILE):
                print('HDR: \rProgress ',int(round((progr/os.path.getsize(self.INFILE))*100)),'%',end='')
            progr = progr + self.chunk
            if not s:
                break
        o.close()
        F.close()
        print('HDR: \rProgress: 100 %')
...
class vui_parameters(object):
    def __init__ (self,t,chroma_bit,video_fmt,full_range,primaries,trc,matrix):
